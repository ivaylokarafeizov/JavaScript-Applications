...
describe('E2E tests', function () {
  ...
  describe('Home Page [ 20 Points ]', () => {
    it('show empty home page [ 5 Points ]', async () => {
      ...
      const titles = await page.$$eval('#welcome-world .game h3', (t) =>
  ...
  describe('CRUD [ 50 Points ]', () => {
    ...
    it('edit should populate form with correct data [ 5 Points ]', async () => {
      ...
      const textArea = await page.$$eval('.container textarea', (t) =>
        t.map((i) => i.value)
      );
  ...
  describe('BONUS: Comments [ 15 Points ]', async () => {
    ...
    it('Guest should not be able to see text "No comments." (with 0 comments) [ 2.5 Points ]', async () => {
      ...
      await page.click(`.game:has-text("${data.title}") >> .details-btn`);
      await page.waitForTimeout(interval);
      const comments = await page.$$eval('details-comments ul li', (t) =>
        t.map((s) => s.textContent)
      );
    ...
    it('No-author can add new comment (test with 2 comments) [ 5 Points ]', async () => {
      ...
      await page.waitForTimeout(interval);
      let allComments = await page.$$eval('.details-comments ul li', (t) =>
        t.map((s) => s.textContent)
      );
      ...
      allComments = await page.$$eval('.details-comments ul li', (t) =>
        t.map((s) => s.textContent)
      );
...
async function setupContext(context) {
  ...
  await handleContext(context, endpoints.catalog, {
    get: mockData.catalog.slice(0, 3),
  });

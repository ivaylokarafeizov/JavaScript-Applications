...
const endpoints = {
  ...
  profile: (id) =>
    `/data/theaters?where=_ownerId%3D%22${id}%22&sortBy=_createdOn%20desc`,
  total: (theaterId) =>
...
describe('E2E tests', function () {
  ...
  describe('Home Page [ 15 Points ]', () => {
    ...
    it('Check home page with 2 theaters [ 2.5 Points ]', async () => {
      ...
      const names = await page.$$eval('.eventsInfo .title', (t) =>
    ...
    it('Check home page info [ 5 Points ]', async () => {
      ...
      const date = await page.$$eval('.eventsInfo .date', (t) =>
        t.map((s) => s.textContent)
      );
      const author = await page.$$eval('.eventsInfo .author', (t) =>
        t.map((s) => s.textContent)
      );
  ...
  describe('CRUD [ 50 Points ]', () => {
    ...
    it('Check details information [ 5 Points ]', async () => {
      ...
      await page.waitForSelector('#events');
      await page.click(
        `.theaters-container > .eventsInfo:has-text("${data.title}") >> .btn-details`
      );

      await page.waitForTimeout(interval);

      const names = await page.$$eval('.detailsInfo h1', (t) =>
        t.map((s) => s.textContent)
      );
      const date = await page.$$eval('.details h4', (t) =>
        t.map((s) => s.textContent)
      );
      const description = await page.$$eval('.details p', (t) =>
        t.map((s) => s.textContent)
      );
    ...
    it('Edit should populate form with correct data [ 5 Points ]', async () => {
      ...
      const inputs = await page.$$eval('.theater-form input, textarea', (t) =>
        t.map((i) => i.value)
      );
  ...
  describe('User Profile Page [ 10 Points ]', async () => {
    ...
    it('Check profile page with 2 theaters [ 5 Points ]', async () => {
      ...
      const titles = await page.$$eval('.board .event-info h2', (t) =>
        t.map((s) => s.textContent)
      );
    ...
    it('Check profile page information [ 2.5 Points ]', async () => {
      ...
      const date = await page.$$eval('.board .event-info h6', (t) =>
        t.map((s) => s.textContent)
      );
      const email = await page.$$eval('.userInfo h2', (t) =>
        t.map((s) => s.textContent)
      );
  ...
  describe('BONUS : Like functionality  [ 15 Points ]', async () => {
    ...
    it('Like button should be hidden(not visible) after a click on it [ 2.5 Points ]', async () => {
      ...
      const [request] = await Promise.all([onRequest, page.click('.btn-like')]);
    ...
    it('Like button should increase total likes by 1 after a click on it [ 5 Points ]', async () => {
      ...
      let likes = await page.$$eval('.likes', (t) =>
        t.map((s) => s.textContent)
      );

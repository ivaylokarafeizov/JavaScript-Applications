...
const rowTemplate = (book) => html` <tr>
  <td>${book.title}</td>
  <td>${book.author}</td>
  <td data-id=${book._id}>
...
const tableTemplate = (context) => html` <table>
  <thead>
    <tr>
      <th>Title</th>
      <th>Author</th>
...
const createFormTemplate = () => html` <form id="add-form">
  <h3>Add book</h3>
  <label>TITLE</label>
...
const editFromTemplate = (book) => html` <form id="edit-form">
...
const layoutTemplate = (context, book) => html` <button
    @click=${context.load}
    id="loadBooks"
  >
    LOAD ALL BOOKS

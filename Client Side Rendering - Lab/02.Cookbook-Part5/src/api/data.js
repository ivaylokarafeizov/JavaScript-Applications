...
const endpoints = {
  ...
  RECENT_RECIPES:
    ...
    '&sortBy=' +
    encodeURIComponent('_createdOn desc'),
  RECIPES: 'data/recipes',
  RECIPE_BY_ID: 'data/recipes/',
  COMMENTS: 'data/comments',
  COMMENTS_BY_RECIPE_ID:
    'data/comments?where=' + encodeURIComponent('recipeId='),
};
...
export async function getRecipes(page = 1) {
  return await api.get(
    endpoints.RECIPE_LIST + `&offset=${(page - 1) * 5}&pageSize=5`
  );
}
...
export async function getCommentsByRecipeId(recipeId) {
  return await api.get(
    endpoints.COMMENTS_BY_RECIPE_ID +

...
const commentsTemplate = (recipe, commentForm, comments) => html` <div
    class="section-title"
  >
  ...
  <div class="comments">
    ${until(
      (async () => commentsList(await comments))(),
      'Loading comments...'
    )}
  </div>`;

const commentFormTemplate = (active, toggleForm) => html` <article
  class="new-comment"
>
  ${active
...
export function showComments(recipe, nav) {
  ...
  function renderTemplate(comments) {
    render(
      commentsTemplate(recipe, createForm(formActive, toggleForm), comments),
      result
    );
  }

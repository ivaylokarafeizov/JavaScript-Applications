...
const homeTemplate = (recentRecipes, goTo) => html` <section id="home">
  <div class="hero">
    <h2>Welcome to My Cookbook</h2>
  </div>
  <header class="section-title">Recently added recipes</header>
  <div class="recent-recipes">
    ${recentRecipes[0] ? recentRecipe(recentRecipes[0], goTo) : ''} ${spacer()}
    ${recentRecipes[1] ? recentRecipe(recentRecipes[1], goTo) : ''} ${spacer()}
...
const recentRecipe = (recipe, goTo) => html` <article
  class="recent"
  @click=${() => goTo('details', recipe._id)}
>
